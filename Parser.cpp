...
{
public:
    OperatorNode(const ASTN *& left, const ASTN *& right) : left(left), right(right){ }
    QVariant evaluate(){
        if (Operator == '+'){
...
{
public:
    AssignmentNode(std::string identifier, const ASTN * & value): value(value), identifier(identifier){}
    QVariant evaluate(){
        double val = (value->evaluate()).toList();
...
{
public:
    FunctionNode(std::string identifier, const ASTN * & values) : identifier(identifier), values(values){}
    QVariant evaluate(){
